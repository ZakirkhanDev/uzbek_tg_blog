...
async def test_fixtures(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin uchun: soat 7 kutmasdan, kunlik fixture postini hoziroq yuboradi (test uchun)."""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("⛔ Bu buyruq faqat adminlar uchun.")
        return
    await update.message.reply_text("⏳ Tekshiryapman...")
    try:
        await post_daily_fixtures(context.application)
        await update.message.reply_text("✅ Tayyor. Kanalga qarang (agar bugun o'yin bo'lmasa, post chiqmaydi).")
    except Exception as e:
        await update.message.reply_text(f"❌ Xatolik: {e}")
