...
from handlers.commands import start, status, natija, league_standings_callback, league_results_callback, fixtures_results_callback
from handlers.admin import (
    post_manual, handle_admin_media, admin_callback, transfer_post, news_post,
    test_fixtures, test_results
)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(league_standings_callback, pattern=r"^(standings|standings:|standings_close)"))
    app.add_handler(CallbackQueryHandler(fixtures_results_callback, pattern=r"^(fx_menu|fx:)"))
    app.add_handler(CallbackQueryHandler(league_results_callback, pattern=r"^(results|results:|results_close)"))
